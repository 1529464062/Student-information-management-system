...
namespace Maticsoft.DAL
{
	...
	public partial class news
	{
		...
		public int Add(Maticsoft.Model.news model)
		{
			...
			strSql.Append("Title,Author,ReleaseTime,Content,RelateFile)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@RelateFile", SqlDbType.VarChar,100)};
			parameters[0].Value = model.Title;
			parameters[1].Value = model.Author;
			parameters[2].Value = model.ReleaseTime;
			parameters[3].Value = model.Content;
			parameters[4].Value = model.RelateFile;
		...
		public bool Update(Maticsoft.Model.news model)
		{
			...
			strSql.Append("RelateFile=@RelateFile");
			strSql.Append(" where Id=@Id");
			SqlParameter[] parameters = {
					new SqlParameter("@Title", SqlDbType.VarChar,100),
					new SqlParameter("@Author", SqlDbType.VarChar,20),
		...
		public Maticsoft.Model.news GetModel(int Id)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 Id,Title,Author,ReleaseTime,Content,RelateFile from news ");
			strSql.Append(" where Id=@Id");
			SqlParameter[] parameters = {
					new SqlParameter("@Id", SqlDbType.Int,4)
			};
		...
		public Maticsoft.Model.news DataRowToModel(DataRow row)
		{
			...
			if (row != null)
			{
				...
				if(row["RelateFile"]!=null)
				{
					model.RelateFile=row["RelateFile"].ToString();
				}
			}
			return model;
		}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select Id,Title,Author,ReleaseTime,Content,RelateFile ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" Id,Title,Author,ReleaseTime,Content,RelateFile ");
			strSql.Append(" FROM news ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
