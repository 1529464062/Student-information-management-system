			...
			if (document.getElementById("studentInfoInsert_Add")) {
				document.getElementById("studentInfoInsert_Add").addEventListener("click", function () {
					$.post("../Handler/studentInfoInsert_Add.aspx", {
						...
						"StudentPhoto" : document.getElementById("studentInfoInsert_StudentPhoto").src,
						...
						"StudentAddress" : document.getElementById("studentInfoInsert_StudentAddress").value.trim()
					}, function (data) {
						if (data == "ok") {
							alert("添加成功");
				...
				$.get("../Handler/studentInfoSelectList.aspx", {
					...
					if (Math.ceil(JsonData.RecordCount / 10) != 0) {
						...
						for (var i = 0; i < JsonData.product.length; i++) {
							...
							tbodyData = tbodyData + "<td><img src='" + JsonData.product[i].StudentPhoto + "' style='height:100px;width:75px'/></td>";
